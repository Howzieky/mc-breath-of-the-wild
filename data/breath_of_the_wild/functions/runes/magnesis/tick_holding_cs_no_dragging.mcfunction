execute unless entity @e[tag=raytrace] run summon minecraft:area_effect_cloud ~ ~ ~ {Duration:10000s,Tags:["raytrace","stopAtMetal"],NoGravity:1b}
...
execute as @e[predicate=breath_of_the_wild:holding_carrot_on_a_stick] at @s run function breath_of_the_wild:tools/start_raytrace
execute as @e[type=#breath_of_the_wild:metal_entities] run data merge entity @s {Glowing:1b}
...
#execute at @e[tag=raytrace] unless entity @e[tag=selectedMagnesisEntity] align xyz positioned ~.5 ~ ~.5 at @e[tag=magnesisFallingBlock,distance=1..] run summon minecraft:area_effect_cloud ~ ~ ~ {Duration:1s,Tags:["fallingBlockJanitor"]}
