...
execute if score galeCounter timers matches 0..4 if entity @s[tag=crouching,tag=!galeReady,predicate=breath_of_the_wild:on_ground,tag=!revaliDisabled] run particle minecraft:instant_effect ^ ^ ^ 1 .1 1 0 5 force
execute if score galeCounter timers matches 0..4 if entity @s[tag=!galeReady,predicate=breath_of_the_wild:on_ground,tag=!revaliDisabled,scores={sneakTime=30..}] at @s run function breath_of_the_wild:abilities/revali/prime_gale
